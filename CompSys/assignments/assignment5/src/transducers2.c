...
int transducers_link_2(stream **out,
  ...
  //for (int i = 0; i < in1->pid_len; i++) {
  //  printf("lol1: %d\n", in1->pids[i]);
  //}
  //for (int i = 0; i < in2->pid_len; i++) {
  //  printf("lol2: %d\n", in2->pids[i]);
  //}
  ...
  memcpy(new_stream->pids + in1->pid_len, in2->pids, sizeof(int) * in2->pid_len);

  new_stream->pids[in1->pid_len + in2->pid_len] = pid;
  new_stream->pid_len = in1->pid_len + in2->pid_len + 1;// + 1;
  *out = new_stream;

  //for (int i = 0; i < new_stream->pid_len; i++) {
  //  printf("lol3: %d\n", new_stream->pids[i]);
  //}

  printf("link 2 new pid: %d\n", pid);
  return 0;
}
...
int transducers_dup(stream **out1, stream **out2,
  ...
  int new_fd = dup(in->read_fd);
  new_stream2->read_fd = new_fd;
