    ...
    )
{
	...
	if (op == isa_ALUOp_mulq) {
	    model_mul(annotation, src, dst);
	    pp_condition(buffer1, state->flags);
            pp_reg_res(buffer2, dst, result);
            note_done_reg_cc(annotation, buffer2, buffer1);

	} else if (op == isa_ALUOp_cmpq) {
	    model_alu(annotation, src, dst, false);
	    pp_condition(buffer1, state->flags);
            note_done_cc(annotation, buffer1);

	} else {
	    model_alu(annotation, src, dst, true);
	    pp_condition(buffer1, state->flags);
            pp_reg_res(buffer2, dst, result);
            note_done_reg_cc(annotation, buffer2, buffer1);
	}
    ...
    )
{
  ...
  #define CASE(__op__) case isa_Opcode_##__op__
    ...
    CASE(mulq_rr): return sim_alu(state, trace, isa_ALUOp_mulq, instr.rr.src, instr.rr.dst, anno);
